vim.g.mapleader = ' ' -- 메인 리더키: Space
vim.g.maplocalleader = '\\' -- 로컬 리더키: Backslash (기본값)

-- 커스텀 리더키들 설정
local function setup_custom_leaders()
  -- Comma(,) 키 - 즐겨찾기 파일 접근용
  ...
  vim.keymap.set('n', "t'", function()
    vim.cmd 'vsplit'
  end, { desc = 'Vertical split (same buffer)' })

  -- t- = horizontal split with same buffer
  vim.keymap.set('n', 't-', function()
    vim.cmd 'split'
  ...
  -- Git 관련 prefix: <leader>g
  vim.keymap.set('n', '<leader>g', '<nop>', { desc = 'Git operations' })

  -- 디버깅 prefix: <leader>d
  vim.keymap.set('n', '<leader>d', '<nop>', { desc = 'Debug operations' })

  -- Emacs-style 키 바인딩
  -- colon과 semicolon 바꾸기 (colon을 더 자주 사용)
  vim.keymap.set('n', ';', ':', { desc = 'Command mode (easier access)' })
...
vim.keymap.set('n', '<leader>bl', ':buffers<CR>', { desc = '[L]ist buffers' })
  ...
  { -- Useful plugin to show you pending keybinds.
    ...
    opts = {
      ...
      spec = {
        ...
        { '<leader>d', group = '[D]ebug operations' },
        { '<leader>?', group = 'Key mappings' },
        -- Comma 키 그룹 (즐겨찾기 파일)
        { ',', group = 'Quick files' },
        -- t 키 그룹 (윈도우 관리)
        { 't', group = 'Window operations' },
      },
    },
  },
